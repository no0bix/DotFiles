...
function sendNotificationTest(testType, e) {
    ...
    sendMessageToBG("showNotificationTest", sendParams).then(() => {
        hideLoading();
    }).catch(error => {
        hideLoading();
        niceAlert(error);
	...
	if (!$("#" + tabName + "Page").length) {
		...
		if (tabName == "welcome") {
			...
			$("#lang paper-item").click(function () {
				...
				// ff patch for dead object errors
			...
			if (await storage.get("accountAddingMethod") == "autoDetect" && $("#showNotificationEmailImagePreview").length) {
				if (chrome.permissions) {
					chrome.permissions.contains({ origins: [Origins.ALL] }, function (result) {
						if (!result) {
							$("#showNotificationEmailImagePreview").get(0).checked = false;
						}
					});
				} else {
					$("#showNotificationEmailImagePreview").get(0).checked = false;
			...
			$("#showNotificationEmailImagePreview").change(async function() {
				var checkbox = this;
				if (checkbox.checked && await storage.get("accountAddingMethod") == "autoDetect") {
					chrome.permissions.request({origins: [Origins.ALL]}, granted => {
						if (granted) {
							// do nothing
							return true;
						} else {
			...
			$("#dndSchedule").click(async function () {
                ...
                const twentyFourHourMode = await storage.get("24hourMode");
				for (var hour = 0; hour < 24; hour++) {
					...
					$time.text(date.formattedTime(twentyFourHourMode));
			...
			$("#hide_count, #showButtonTooltip").change(async function () {
				await waitForStorageSync();
				bg.updateBadge(lsNumber("unreadCount"));
			});
            ...
            if (await storage.get("playVideosInsideGmail")) {
                $("#playVideosInsideGmail")[0].checked = true;
            }
			...
			$("#playVideosInsideGmail").change(function() {
				if (this.checked) {
                    chrome.permissions.request({permissions: ["webRequest"]}, async function(granted) {
                        if (granted) {
                            await storage.enable("playVideosInsideGmail");
                    ...
                    storage.disable("playVideosInsideGmail");
                }
			});
			...
			$("#useBasicHTMLView").change(async function () {
				...
				if (this.checked) {
					openGenericDialog({
						...
						if (response == "cancel") {
							const account = await getFirstActiveAccount(bg.accounts);
			...
			$("#accountAddingMethod paper-radio-button").click(async () => {
				await waitForStorageSync();
				await resetSettings(bg.accounts);
				await alwaysPromise(pollAndLoad({showNotification:false, refresh:true}));
				await initDisplayForAccountAddingMethod();
                bg.restartCheckEmailTimer(true);
                if (await storage.get("accountAddingMethod") == "autoDetect") {
                    initDuplicateAccountDetection();
                } else {
                    chrome.alarms.clear(Alarms.DUPLICATE_ACCOUNT_DETECTION);
                }
			});
			...
			$("#pollingInterval paper-listbox").on("iron-activate", async function(e) {
				...
				if (pollingInterval == "realtime" && !DetectClient.isChrome()) {
					...
					new Promise(async (resolve, reject) => {
						if (previousPollingInterval != "realtime" && pollingInterval == "realtime") {
                            ...
                            await asyncForEach(bg.accounts, async (account) => {
								if (!await account.isBeingWatched()) {
									var pushNotificationPromise = account.enablePushNotifications();
									promises.push(pushNotificationPromise);
								}
							});
			...
			$("#addAccount").click(async function () {
				...
				if ((tokenResponses && tokenResponses.length) || DetectClient.isFirefox()) {
...
function pollAndLoad(pollAccountsParams) {
	console.log("pollAndLoad");
	showLoading();
	
	return bg.pollAccounts(pollAccountsParams).then(() => {
...
async function loadLabels(params) {
	...
	if (account) {
		...
		// detect dead object and reload
		try {
			if (DetectClient.isFirefox()) {
				monitorLabelsEnabled.toString();
			}
		} catch (error) {
			chrome.runtime.reload();
		}
		...
		var systemLabelsOpened = !await account.isUsingGmailCategories() || monitorLabelsEnabled.indexOf(SYSTEM_INBOX) != -1 || monitorLabelsEnabled.indexOf(SYSTEM_IMPORTANT) != -1 || monitorLabelsEnabled.indexOf(SYSTEM_IMPORTANT_IN_INBOX) != -1 || monitorLabelsEnabled.indexOf(SYSTEM_ALL_MAIL) != -1;
		...
		account.getLabels(params.refresh).then(async response => {
			if (response.labels) {
                await asyncForEach(response.labels, async label => {
...
async function loadAccountsOptions(loadAccountsParams) {
	...
	$monitorLabels.off("change").on("change", ".monitoredLabelCheckbox", async function() {
		...
		account.saveSetting("monitorLabel", getEnabledLabels()).then(() => {
...
async function initDisplayForAccountAddingMethod() {
	...
	if ($pollingInterval.length) {
		$pollingInterval[0].select(await calculatePollingInterval(bg.accounts));
	}
}
		...
		account.getEmails().then(() => {
			...
			account.syncSignInId().then(() => {
				...
				if (poll == "realtime" || poll <= seconds(30)) {
					alwaysPromise(account.enablePushNotifications()).then(async () => {
						await initDisplayForAccountAddingMethod();
						bg.restartCheckEmailTimer(true);
					});
				}
...
$(document).ready(function() {
	...
	(async () => {
		...
		function gmailtest(account) {
			...
			setTimeout(function(){
				ajax(account.getMailUrl()).then(data => {
					hideLoading();
					...
					hideLoading();
					niceAlert("error: " + jqXHR.statusText);
				});
			}, 3000)
		}
		...
		$("#testGmailAT").click(async function() {
			const account = await getFirstActiveAccount(bg.accounts);
			gmailtest(account);
		});
		...
		$("#testMarkAsRead").click(async function() {
			const account = await getFirstActiveAccount(bg.accounts);
		...
		$("#flushGmailAT").click(async function() {
			const account = await getFirstActiveAccount(bg.accounts);
