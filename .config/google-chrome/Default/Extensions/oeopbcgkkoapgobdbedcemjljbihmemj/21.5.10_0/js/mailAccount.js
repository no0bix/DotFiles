...
function MailAccount() {
   ...
   this.getEmails = function(params) {
	   ...
	   return new Promise(async (resolve, reject) => {
		   ...
		   if (accountAddingMethod == "autoDetect") {
			   fetchEmailsByLabel({monitorLabels:accountMonitorLabels, monitorLabelsIndex:0, feedsArrayInfo:[]}).then(fetchEmailsResponse => {
				   var inboxFeed;
				   ...
				   var allPromises = mainCategoryFeeds.reduce((sequence, mainCategoryFeed) => {
			   ...
			   function getInitialMessages(params) {
				   return new Promise(async (resolve, reject) => {
					   ...
					   getMessagesByList({monitoredLabels:accountMonitorLabels, historyId:params.historyId}).then(newestMailArray => {
			   ...
			   that.getLabels(params.refresh).then(async () => {
				   that.error = null;
				   that.errorCode = null;
				   ...
				   if (historyId) {
					   ...
					   getMessagesByHistory(getMessagesByHistoryParams).then(messagesByHistoryResponse => {
						   return new Promise((resolve, reject) => {
							   ...
							   if (histories && histories.length) {
								   ...
								   fetchMessagesByIds(messagesToFetch).then(async fetchMessagesByIdsResponse => {
									   ...
									   new Promise((resolve, reject) => {
										   if (that.unreadCount >= MAX_EMAILS_TO_FETCH) {
											   fetchUnreadCount(monitoredLabels, true).then(fetchedUnreadCount => {
						   ...
						   if (errorResponse.jreason == JError.HISTORY_INVALID_OR_OUT_OF_DATE || errorResponse.jreason == JError.TOO_MANY_HISTORIES || errorResponse == JError.EXCEEDED_MAXIMUM_CALLS_PER_BATCH) {
							   ...
							   getInitialMessages({account:that, getEmailsResponse:getEmailsResponse}).then(response => {
								   resolve(response);
							   }).catch(error => {
								   reject(error);
							   });
   ...
   function fetchUnreadCount(labelIds, fetchWithInboxCount) {
	   ...
	   return new Promise(async (resolve, reject) => {
		   ...
		   mygapiClient.HttpBatch.execute({oauthRequest:oAuthForEmails, email:mailAddress}).then(async batchResponse => {
			   let unreadCount = 0;
		   ...
		   ajaxBasicHTMLGmail(ajaxParams).then(data => {
			   if (params.action == MailAction.REPLY) {
				   ...
				   if (data && (data.indexOf("#FAD163") != -1 || data.toLowerCase().indexOf("your message has been sent") != -1)) {
					   ...
					   reject("Send error: Could not confirm action");
				   }
			   } /* else {
   ...
   function executeMailAction(params) {
	   ...
	   return new Promise(async (resolve, reject) => {
		   if (accountAddingMethod == "autoDetect") {
			   ...
			   getGmailAT().then(gmailAT => {
				   return executeGmailHttpAction(params);
			   }).then(response => {
   				   that.getEmails().then(async () => {
	   ...
	   if (emailSettings) {
		   ...
		   if (accountEmailSettings) {
			   ...
			   if (accountEmailSetting != undefined) {
				   ...
				   // legacy to rename image.url propery to just imageUrl
				   if (attributeName == "profileInfo" && accountEmailSetting && accountEmailSetting.image && accountEmailSetting.image.url) {
						let newSettingsObj = {};
						if (accountEmailSetting.displayName) {
							newSettingsObj.displayName = accountEmailSetting.displayName;
						}
						if (accountEmailSetting.image) {
							newSettingsObj.imageUrl = accountEmailSetting.image.url;
						}
						await that.saveSetting(attributeName, newSettingsObj);
				   }
				   // end legacy
   ...
   this.fetchGmailSettings = function(params) {
	   params = initUndefinedObject(params);
	   return that.ensureMailAddressIdentified().then(() => {
		   return new Promise(async (resolve, reject) => {
			   ...
			   if (lastFetchedGmailSettings && lastFetchedGmailSettings.diffInDays() > -FETCH_INTERVAL_IN_DAYS) { // !params.refresh && 
				   ...
				   var jqxhr = $.ajax({
					   ...
					   console.log("gmail settings", jqXHR, textStatus);
					   ...
					   if (data) {
						   ...
						   that.saveSetting("gmailSettings", {conversationViewMode:conversationViewMode, tabs:tabs}).then(() => {
                               resolve();
                           });
					   } else {
						   reject(textStatus);
