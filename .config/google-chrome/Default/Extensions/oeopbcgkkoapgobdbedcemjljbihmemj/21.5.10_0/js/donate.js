...
function isZeroDecimalCurrency(currencyCode) {
	...
	if (zeroDecimalCurrencies.indexOf(currencyCode.toLowerCase()) != -1) {
		return true;
	}
}
...
function createStripeSource(params) {
	showLoading();
	...
	*/
	...
	return new Promise((resolve, reject) => {
		...
		ajax({
			type: "POST",
			url: "https://apps.jasonsavard.com/paymentSystems/stripe/ajax.php",
			data: data
			...
			sendGA(params.type, 'error with token: ' + jqXHR.responseText);
			reject(jqXHR.responseText);
		});
	});
}

function validateStripeSource(type) {
...
function initPaymentProcessor(price) {
	if (donateButtonClicked == "paypal") {
		...
		ajax({type:"post", url:"https://apps.jasonsavard.com/paymentSystems/coinbase/ajax.php", data:data}).then(response => {
			...
			sendGA("coinbase", 'error with createCoinbaseCharge');
		});
	} else {
		openGenericDialog({
			title: getMessage("theresAProblem"),
...
function isPayPalSubscriptionsSupported() {
	function isInThisLang(thisLang) {
		return lang.indexOf(thisLang) != -1 || chrome.i18n.getUILanguage().indexOf(thisLang) != -1;
	}
...
$(document).ready(() => {
	...
	(async () => {
		...
		email = await getFirstActiveEmail( accountsWithoutErrors );
		ensureEmail(true);
