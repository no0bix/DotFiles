		...
		if (desktopNotification) {
			getDNDState().then(dndState => {
				if (dndState) {
					...
					ensureDoNotShowNotificationIfGmailTabOpenTest(params).then(async passedDoNotShowNotificationIfGmailTabOpenTest => {
...
function openNotification(params) { // expected args: options, buttonsWithValues, emails
	return new Promise(async (resolve, reject) => {
		// remove previous notifications
		clearRichNotification(richNotifId).then(async () => {
...
function refreshAccounts(hardRefreshFlag) {
	return new Promise(async (resolve, reject) => {
		
		function hardRefresh() {
			pollAccounts({refresh:true}).then(async () => {
                await mailUpdate();
                resolve();
			}).catch(error => {
				reject(error);
		...
		if (hardRefreshFlag) {
			...
			if (activeAccounts && activeAccounts.length >= 1 && !accountsWithErrors) {
				getAllEmails({accounts:activeAccounts, refresh:true}).then(async () => {
					await mailUpdate();
					resolve();
				});
			} else {
