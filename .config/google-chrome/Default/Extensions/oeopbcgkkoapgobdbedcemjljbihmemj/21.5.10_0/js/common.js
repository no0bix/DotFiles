	...
	this.execute = function(sendOAuthParams) {
		return new Promise(async (resolve, reject) => {
			...
			if (httpRequests.length) {
				...
				sendOAuthParams.oauthRequest.send({tokenResponse:tokenResponse, type:"POST", url: GmailAPI.DOMAIN + "/batch/gmail/v1", contentType:"multipart/mixed", boundary:boundary, headers:{"Content-Length":totalContentLength}, processData:false, data:data}).then(sendResponse => {
...
function OAuthForDevices(params) {
	...
	function initError(jqXHR, tokenResponse, oauthAction) {
		...
		error.textStatus = jqXHR.statusText;
		error.tokenResponse = tokenResponse;
		error.oauthAction = oauthAction;
		...
		return new Promise(async (resolve, reject) => {
			...
			if (tokenResponse) {
				ensureToken(tokenResponse).then(response => {
					...
					logError("error generating url", error);
					reject(error);
				});
			} else {
	...
	function ensureToken(tokenResponse) {
		return new Promise((resolve, reject) => {
			if (tokenResponse.chromeProfile) {
				...
				chrome.identity.getAuthToken(getAuthTokenParams, token => {
					if (chrome.runtime.lastError) {
						reject(chrome.runtime.lastError.message);
					} else {
				...
				refreshToken(tokenResponse).then(response => {
					resolve(response);
				}).catch(error => {
					reject(error);
				});
	...
	function refreshToken(tokenResponse) {
        ...
        return ajax({
            ...
            let error = initError(jqXHR, tokenResponse, "refreshToken");
		...
		return new Promise(async (resolve, reject) => {
			...
			if (tokenResponse) {
				ensureToken(tokenResponse).then(response => {
					...
					console.info("in error", error, tokenResponse);
					reject(error);
				});
			} else {
				var errorEmail;
		...
		return new Promise((resolve, reject) => {
			if (params.code) {
				...
				new Promise(async (resolve, reject) => {
					if (params.refetch) {
						if (params.userEmail) {
							let tokenResponse = await that.findTokenResponse({ userEmail: userEmail });
							removeCachedAuthToken(tokenResponse.access_token).then(() => {
								resolve();
							}).catch(error => {
								console.error(error);
								resolve();
					...
					let tokenResponse = { chromeProfile: true };
					let getAuthTokenParams = { interactive: true, scopes: SCOPE.split(" ") };
					chrome.identity.getAuthToken(getAuthTokenParams, token => {
						if (chrome.runtime.lastError) {
							console.info("tring again: " + chrome.runtime.lastError.message, token);
							...
							chrome.identity.getAuthToken(getAuthTokenParams, token => {
								if (chrome.runtime.lastError) {
									console.error(chrome.runtime.lastError.message, token);
									reject(chrome.runtime.lastError.message);
								} else {
									tokenResponse.access_token = token;
			...
			return that.getUserEmail(tokenResponse, sendOAuthRequest).then(async response => {
