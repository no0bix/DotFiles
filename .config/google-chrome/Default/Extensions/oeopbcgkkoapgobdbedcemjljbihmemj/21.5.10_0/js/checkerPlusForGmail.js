...
function ensureContactsWrapper(accounts) {
	return new Promise(async (resolve, reject) => {
		if (await storage.get("showContactPhoto")) {
		    ...
		    oAuthForContacts.ensureTokenForEmail(emailsWithUnreadMail).then(function(response) {
		    	resolve(response);
	...
	this.startAnimation = function(params) {
		return new Promise(async (resolve, reject) => {
			if (typeof OffscreenCanvas != "undefined" || typeof document != "undefined") {
				...
				reject(new Error("Missing API to animate icon!"));
			}
		}).catch(error => {
			console.error(error);
			self.stopAnimation(params);
...
function updateContacts() {
	return new Promise(async (resolve, reject) => {
		...
		if (contactsData) {
			...
			Promise.all(fetchContactPromises).then(async responses => {
