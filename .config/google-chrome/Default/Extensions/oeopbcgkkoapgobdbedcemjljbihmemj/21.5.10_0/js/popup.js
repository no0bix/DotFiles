...
async function initTabs(email) {
	...
	if (account) {
		...
		account.getLabels().then(async response => {
	...
	$(document).ready(function() {
        ...
        (async () => {
            ...
            if (popupView == POPUP_VIEW_CHECKER_PLUS) {
                ...
                var urlPrefix = "https://mail.google.com/mail/mu/mp/?mui=checkerPlusForGmail&hl=" + await storage.get("language");
                    ...
                    .off("load").on("load", function() {
                            ...
                            setTimeout(async () => {
                                ...
                                if (!currentTabletFrameEmail) {
                                    ...
                                    initTabs(await getFirstActiveEmail(accounts));
                                }
                            }, 500);
...
async function getBGObjects() {
	...
	totalUnreadCount = lsNumber("unreadCount");
    
    twentyFourHourMode = await storage.get("24hourMode");
...
function setContactPhoto(params, imageNode) {
	...
	// contact photo
	getContactPhoto(params, async function(response) {
...
function setPopupBgColor(color) {
	if (color) {
		addSkin({
			id: "background-color",
			css: "body:not(.background-skin) #inboxSection, body:not(.background-skin) #inboxSection app-header-layout #main-header-toolbar {background-color:" + color + " !important}"
		});
	}
}
...
function updateUnreadCount(offset, $mail) {
	...
	let $account = $mail.closest(".account");
	let account = $account.data("account");
	
	let $unreadCount = $account.find(".unreadCount");
	let unreadCount = $unreadCount.data("count");
	if (unreadCount == undefined) {
		unreadCount = account.unreadCount;
	}
...
function initOpenEmailEventListeners() {
	...
	$("#moveLabel").click(function() {
		...
		showLoading();
		mail.account.getLabels().then(async response => {
			hideLoading();
...
async function resizePopup() {
	...
	if (fromToolbar) {
		await bgObjectsReady;
			
        getZoomFactor().then(async function(zoomFactor) {
...
function setMailMessage($openEmailMessages, mail, message) {
	...
	if (message.date) {
		dateStr = message.date.displayDate({relativeDays:true, twentyFourHourMode:twentyFourHourMode});
		$message.find(".date")
			.data("data", dateStr)
							...
							if (contentId && !/attachment\;/.test(contentDisposition)) {
								...
								queuedFile.fetchPromise.then(function(response) {
									...
									$messageBody.find("img").each(function() {
										if ($(this).attr("src") && $(this).attr("src").indexOf(FOOL_SANITIZER_CONTENT_ID_PREFIX + contentId) != -1) {
...
function openOtherMail($mail, direction) {
	...
	if ($nextMail && $mail.data("mail").account && $nextMail.data("mail").account && $mail.data("mail").account.id == $nextMail.data("mail").account.id) {
...
function refresh(hardRefresh) {
	return new Promise((resolve, reject) => {
        ...
        sendMessageToBG("refreshAccounts", hardRefresh).then(() => {
            accounts = bg.accounts;
            resizePopup();
...
async function prepareMarkAllAsX($account, account, action) {
	...
	if (action == "markAsRead") {
		...
		tooManyAlternativeButton = getMessage("archiveLinkTitle");
		tooManyMarkAsX = getMessage("archiveLink");
	}
...
function openComposeSection(params) {
	...
	$fetchContacts.off().click(function() {
		chrome.permissions.request({ origins: [Origins.CONTACT_PHOTOS] }, granted => {
			if (granted) {
				oAuthForContacts.openPermissionWindow(account.getAddress());
			}
		});
	});
		...
		.click(function() {
			...
			ensureRecordingIsSaved({autoplay:false}).then(async function() {
				...
				if (!await storage.get("donationClicked") && await storage.get("_sendAttachmentTested")) {
					...
					setTimeout(async function() {
						account.sendEmail(sendEmailParams).then(async function() {
							showMessage("Sent");
...
async function renderAccounts() {
    ...
    await asyncForEach(activeAccounts, async (account, accountIndex, thisArray) => {
		...
		if (accountIndex != 0 && accountIndex == (thisArray.length-1) && !account.hasBeenIdentified()) {
			...
			$account.find(".accountHeader").css("background-color", await account.getSetting("accountColor"));
			...
			$account.find(".accountOptionsMenuButton")
				.one("mousedown", async function() { // MUST use .one because mousedown will also be called when menu items *inside the dropdown all are also clicked
						...
						$account.find(".setAccountIcon")
							.click(async function() {
								...
								if (await donationClicked("setAccountIcon")) {
                                    ls["emailAccountRequestingOauth"] = account.getAddress();
									oAuthForProfiles.openPermissionWindow(account.getAddress());
								}
							})
						;
					}
					...
					$account.find(".ignore")
						.click(async function() {
							showLoading();
							await account.remove(oAuthForEmails, bg.accounts);
...
function renderMails(params) {
	...
	mails.some(function(mail, mailIndex) {
		...
		$mail.find(".date").text(mail.getDate(twentyFourHourMode));
...
function showSkinsDialog() {
	...
	Controller.getSkins().then(async skins => {
        ...
        const skinsEnabled = await storage.get("skinsEnabled");
			...
			.on("click", ".addButton", async function (e) {
				...
				if ($addButton.hasClass("selected")) {
                    ...
                    if (skinsEnabled) {
						...
						showMessage(getMessage("disabledSkinsUseEnable"));
					}
				} else {
		...
		if (await storage.get("skinsEnabled")) {
		...
		$dialog.find(".disableSkins").off().on("click", async function() {
			await storage.set("skinsEnabled", false);
			location.reload();
		});

		$dialog.find(".enableSkins").off().on("click", async function() {
			await storage.set("skinsEnabled", true);
			location.reload();
		});
...
async function init() {
	...
	var activeAccounts = await getActiveAccounts(accounts);
	if (activeAccounts.length >= 2 && await storage.get("drawer") != "closed") {
	...
	$body.addClass(await storage.get("displayDensity"));
	...
	if (await storage.get("skinsEnabled") && skinsSettings) {
		//pageVisible.then(() => {
			...
			let popupBgColor = await storage.get("popup-bg-color");
	...
	$body
		...
		.keypress(function(e) {
			...
			if (isFocusOnInputElement()) {
				...
				if (keydown(e, 'c')) {
					...
					if (!isComposeView()) {
						if ($selectedMail.length) { // found unread email so open the email
                            ...
                            getFirstActiveAccount(accounts).then(firstActiveAccount => {
                                firstActiveAccount.openInbox();
                                closeWindow({source:"openInboxShortcutKey"});
                            })
						}
					}
				} else if (keydown(e, 'j')) { // j = next/down
					if (isEmailView()) {
						$("#nextMail").click();
					}
				} else if (keydown(e, 'k')) { // k = prev/up
					if (isEmailView()) {
						$("#prevMail").click();
					}
				...
				} else if ((keyboardException_R == "reply" && keydown(e, 'r')) || keydown(e, 'a')) {
				...
				} else if (keydown(e, 'I') || (keyboardException_R == "markAsRead" && keydown(e, 'r'))) {
					if (isEmailView()) {
						$("#markAsRead").click();
					} else {
	...
	$("#maximize").mousedown(async function(e) {
		if (isCtrlPressed(e)) {
			...
			if (currentAccount) {
				...
				if (tabletViewUrl) {
					var messageId = extractMessageIdFromOfflineUrl(tabletViewUrl);
					if (messageId) {
	...
	if (await storage.get("quickComposeEmail")) {
		...
		var contactPhotoParams = $.extend({}, {
            useNoPhoto: true,
            account: await getFirstActiveAccount(accounts),
	...
	if (isDetached && !await storage.get("popoutMessage") && !previewMailId && location.href.indexOf("action=getUserMediaDenied") == -1 && location.href.indexOf("action=getUserMediaFailed") == -1 && location.href.indexOf("source=oauth") == -1) {
		polymerPromise2.then(() => {
	...
	if (accounts.length == 0 || (accounts.length == 1 && accounts[0].error && accounts[0].error != "timeout" && accounts.first().getMailUrl().indexOf("/mail/") != -1)) {
		...
		polymerPromise2.then(() => { // polymerPromise2 required to fix paper-dialog-scrollable issue with 0 height
			...
			if (accounts.length == 1 && accounts.first().error) {
				...
				if (DetectClient.isChrome()) {
					let $iframe = $("<iframe>", {
						src:	"https://jasonsavard.com/tools/third-party-cookies.htm",
						style: 	"border:0; height: 30px; width: 100%"
					});
					$dialog.find("#signInErrorInstructions").append($iframe);
				}
				*/
			} else {
				if (accountAddingMethod == "autoDetect") {
