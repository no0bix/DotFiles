  ...
  (function() {
    var Media = function() {
      ...
      this.init = function() {
        ...
        function mediaDetectListener(m) {
          ...
          function f(m) {
            ...
            if (
              !_b(fvdSingleDownloader.Prefs.get('fvd.original_filename')) &&
              m.ext == 'swf'
            ) {
        ...
        chrome.tabs.query(
          ...
          function(tabs) {
            if (tabs.length > 0) {
              ...
              chrome.tabs.create(
                { active: true, url: chrome.extension.getURL('/options.html') },
                function(tab) {}
              );
            }
          }
        ...
        fvdSingleDownloader.Utils.Async.chain([
          function(chainCallback) {
            ...
            if (fvdSingleDownloader.noYoutube == false) {
              fvdSingleDownloader.FvdMobile.downloadMedia(media, function(
                result
              ) {
                if (!result) {
                  chainCallback();
          ...
          function(chainCallback) {
            ...
            if (flag_download == 1) {
              ...
              fvdSingleDownloader.Downloader.start(
                media.id,
                media.url,
                file_name,
          ...
          function(chainCallback) {
            ...
            fvdSingleDownloader.Utils.getActiveTab(function(tab) {
              fvdSingleDownloader.ContentScriptController.processMessage(
                tab.id,
                {
      ...
      this.stopDownload = function(media) {
        ...
        if (media.downloadId) {
          fvdSingleDownloader.Downloader.stop(media.downloadId, function(
            error
          ) {
            if (error) {
              console.log(fvdSingleDownloader.Downloader.getError());
          ...
          fvdSingleDownloader.Media.Storage.setData_Attribute(
            media.tabId,
            media.id,
            'status',
            'stop'
      ...
      this.startRecord = function(media, callback) {
        ...
        fvdSingleDownloader.Recorder.start(media.hash, media.playlist, function(
          error,
          countTSFiles,
          sizeOfVideo,
          status
          ...
          if (error) {
            ...
            fvdSingleDownloader.Media.Storage.setTwitch(
              media.hash,
              status,
              sizeOfVideo
            );
      ...
      this.stopRecord = function(media) {
        ...
        fvdSingleDownloader.Recorder.stop(media.hash, function(
          error,
          file,
          status
        ) {
      ...
      this.startCombineDownload = function(media, callback) {
        fvdSingleDownloader.Streamer.start(
          media.hash,
          function(rez) {
            if (rez.msg === 'start') {
              fvdSingleDownloader.Media.Storage.setStream(rez.hash, {
                status: 'start'
              });
              chrome.extension.sendMessage({
                subject: 'mediaStream',
      ...
      function saveCombineFileSys(hash, size, url, callback) {
        ...
        if (media) {
          try {
            ...
            if (flag_download == 1 && !chrome.downloads) flag_download = 5; // ? ????
            var met = _b(
              fvdSingleDownloader.Prefs.get('fvd.original_filename')
            );
            var file_name;
            if (met) {
              // ???????
