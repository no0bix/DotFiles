(function() {
  var Popup = function() {
    ...
    const URL_RATE =
      'https://chrome.google.com/webstore/detail/flash-video-downloader/aiimdkdngfcipjohbjenkahhlhccpdbc/reviews';
    ...
    function str_download_size(size) {
      if (size < 1073741824)
        return fvdSingleDownloader.Utils.bytesToMb(size) + 'MB';
    ...
    function buildThreadItemError(media) {
      ...
      var item = document
        .getElementById('download_error_template')
        .cloneNode(true);
      item.removeAttribute('id');
      fbc('download_title').textContent = media.title;
      fbc('download_button').addEventListener(
        'click',
        function(event) {
    ...
    function buildThreadItem(media) {
      ...
      var item = document
        .getElementById('download_item_template')
        .cloneNode(true);
      ...
      if (_b(fvdSingleDownloader.Prefs.get('fvd.original_filename'))) {
        fbc('download_url').textContent = media.filename
          ? media.filename.substr(0, 30) + '...'
          : media.displayName.substr(0, 30) + '...';
      } else {
        fbc('download_url').textContent =
          media.displayName.substr(0, 30) + '...';
      }
      ...
      fbc('removeLink').addEventListener(
        'click',
        function(event) {
    ...
    function show_rate_message() {
      var showMessageRate = fvdSingleDownloader.Prefs.get(
        'fvd.rate_message.show'
      );
      ...
      if (showMessageRate == 0) {
        ...
        if (
          now - fvdSingleDownloader.Prefs.get('install_time') >
          INTERVAL_TO_DISPLAY_RATE
        ) {
    ...
    this.init = function() {
      ...
      if (elem) {
        if (language == 'ru') {
          elem.setAttribute(
            'href',
            'http://nimbus.everhelper.me/nimbus-note-windows-ru.php'
          );
        }
      }
          ...
          function() {
            ...
            if (k == 0) {
              fvdSingleDownloader.Prefs.set(
                'popup.display_slow_download_hint',
                true
              );
            }
      ...
      if (elem)
        elem.addEventListener(
          'click',
          function() {
            chrome.extension.sendMessage({ action: 'SettingOptions' }, function(
              response
            ) {});
          },
          false
      ...
      if (elem) {
        if (
          now - fvdSingleDownloader.Prefs.get('install_time') <
          INTERVAL_TO_DISPLAY_WRITE_REVIEW
        ) {
      ...
      setTimeout(function() {
        var box1 = document
          .getElementById('captureDownloadHint')
          .getBoundingClientRect();
        if (box1.height == 0) {
          document
            .getElementById('download_item_container')
            .setAttribute('style', 'height: 456px');
        }
      }, 100);
    };
    ...
    function show_sovet() {
      var showSovet = _b(
        fvdSingleDownloader.Prefs.get('fvd.show_sovet_smart_pause')
      );
      ...
      if (showSovet && !fvdSingleDownloader.MainButton.isSmartPause && elem) {
        ...
        elem.addEventListener(
          ...
          function(event) {
            self.navigate_url(
              'https://chrome.google.com/webstore/detail/smart-pause-for-youtube/dcflkimagfnicklojfonbbcppnikogih'
            );
            event.stopPropagation();
          },
          false
    ...
    this.refreshTopHints = function() {
      ...
      if (
        _b(fvdSingleDownloader.Prefs.get('popup.display_slow_download_hint'))
      ) {
      ...
      fvdSingleDownloader.Utils.getActiveTab(function(tab) {
        if (!tab) {
          ...
          if (
            ...
            fvdSingleDownloader.MainButton.isYoutubeUrl(tab.url)
          ) {
            // youtube
    ...
    this.rebuildThreadsList = function() {
      ...
      document.getElementById('multiple_download_block').addEventListener(
        'click',
        function(event) {
