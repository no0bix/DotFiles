...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__ = __webpack_require__(1);
...
_asyncToGenerator(function* () {
	...
	let timer;
	let statsOpen = false;
	const settings = yield __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].many(__WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_detectOverlays, __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_tabswithoverlay, __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_numOfBlocks, __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_pause, __WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_OneTimeEvents).get();
	const whitelist = yield __WEBPACK_IMPORTED_MODULE_3__whitelistManager__["a" /* WhitelistManager */].get();
	chrome.tabs.query({ active: true, lastFocusedWindow: true }, (() => {
		var _ref2 = _asyncToGenerator(function* (tabs) {
			let tab = tabs[0];
			const reqUrl = new URL(location.href);
			const siteParam = reqUrl.searchParams.get('site');

			if (siteParam) {
				// for automations purposes
				//this condition is for security reasons, we want to make sure that the page is from our extension
				if (Object(__WEBPACK_IMPORTED_MODULE_1__utils__["f" /* getBrowser */])() == 'FF' && location.hostname === browser.tabs.getCurrent() || location.hostname === chrome.runtime.id) {
					tab = {
						'url': siteParam
					};
				}
			...
			Object(__WEBPACK_IMPORTED_MODULE_1__utils__["k" /* getTodayBlockCount */])(__WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */], host, function (result) {
				let resultMsg = $('#result-msg');
				let count = result ? result * 1 : 0; // string -> number
	...
	$('#report-over').on('click', function () {
		...
		__WEBPACK_IMPORTED_MODULE_2__PBStorage_PBStorageSync__["a" /* default */].pb_detectOverlays.get().then((() => {
			var _ref3 = _asyncToGenerator(function* (isActive) {
				const isFFP = yield Object(__WEBPACK_IMPORTED_MODULE_1__utils__["m" /* isFireFoxIncognito */])();

				if (isActive) {
					if (!isFFP) {
						Object(__WEBPACK_IMPORTED_MODULE_1__utils__["q" /* sendToActiveTab */])('manual-remove-overlay', { source: 'menu' });
					} else {
						Object(__WEBPACK_IMPORTED_MODULE_1__utils__["q" /* sendToActiveTab */])('overlay-not-active-incognito', { source: 'menu' });
					}
				} else {
					Object(__WEBPACK_IMPORTED_MODULE_1__utils__["q" /* sendToActiveTab */])('overlay-not-active', { source: 'menu' });
				}
