...
const pop = (() => {
    ...
    return {
      ...
      clear(tid) {
        hash[tid] = {
          ...
          u: (hash[tid] || {}).u || null,
          rd: (hash[tid] || {}).rd || '',
          to: ["exthead"], fr: false, aj: (hash[tid] || {}).aj || false,
  ...
  if (wr.OnBeforeSendHeadersOptions && wr.OnBeforeSendHeadersOptions.hasOwnProperty('EXTRA_HEADERS')) {
    beforeSendOptions.push('extraHeaders');
  }
  wr.onBeforeSendHeaders.addListener(onBeforeSendHeaders, repertuar, beforeSendOptions);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__PBStorage_PBStorageSync__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__const__ = __webpack_require__(0);
...
/* harmony default export */ __webpack_exports__["a"] = (function (pop) {
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.name) {
      ...
      case "NotificationOptIn":
        Settings.pb_detectOverlays = true;
        __WEBPACK_IMPORTED_MODULE_5__PBStorage_PBStorageSync__["a" /* default */].pb_detectOverlays.set(true);
      ...
      case "welcomeOptOut":
        Settings.pb_detectOverlays = false;
        __WEBPACK_IMPORTED_MODULE_5__PBStorage_PBStorageSync__["a" /* default */].pb_detectOverlays.set(false);
  ...
  const retension = new __WEBPACK_IMPORTED_MODULE_8__utils__["b" /* Retension */]({
    ...
    GAEvents: {
      Retentoin: function (xDay) {
        trackGAEvent({ category: "General", action: "active user", label: extVersion, dimmesntions: {
            dimension5: pop.isopt() ? 1 : 0,
            dimension6: xDay
          } }, true);
      },
      Install: function () {
  ...
  __WEBPACK_IMPORTED_MODULE_5__PBStorage_PBStorageSync__["a" /* default */].all.update(settings => {
    ...
    settings.pb_whitelistManager = __WEBPACK_IMPORTED_MODULE_7__whitelistManager__["b" /* WhitelistUpdater */].init({ whitelist: settings.pb_whitelistManager }) || __WEBPACK_IMPORTED_MODULE_7__whitelistManager__["a" /* WhitelistManager */].init({ userWhitelist: settings.pb_whitelist });
    ...
    settings.pb_detectOverlays = settings.pb_detectOverlays || false; //default opt out

    //a user is new, lets set GDPR to false so he will never see it
    if (isFreshInstall) {
      settings.pb_OneTimeEvents.GDPR = false;
      settings.pb_OneTimeEvents.Welcome = true;

      pop.optout(); //DEFAULT OPT OUT

      chrome.tabs.create({ url: chrome.extension.getURL('welcome.html#n') });
    }
    //user is updated but never had a GPRD flag before, lets show him
    else {
        if (settings.pb_OneTimeEvents.GDPR == undefined) {
          settings.pb_OneTimeEvents.GDPR = true;
        }

        if (
        //if the browser is firefox
        Object(__WEBPACK_IMPORTED_MODULE_8__utils__["f" /* getBrowser */])() == 'FF' && (
        //if we never showed the user the opt in/opt out welcome page
        //we are doing a default opt out for existing users and let them choose 
        !settings.pb_OneTimeEvents.Welcome || !pop.isopt() && !settings.pb_OneTimeEvents['Welcome_' + extVersion])) {
          settings.pb_OneTimeEvents.Welcome = true;
          settings.pb_OneTimeEvents['Welcome_' + extVersion] = true;
          settings.pb_detectOverlays = false;

          pop.optout(); //DEFAULT OPT OUT
          chrome.tabs.create({ url: chrome.extension.getURL('welcome.html#u') });
        }
      }
  ...
  chrome.contextMenus.onClicked.addListener((() => {
    var _ref = _asyncToGenerator(function* (info, tab) {
      const isFFP = yield Object(__WEBPACK_IMPORTED_MODULE_8__utils__["m" /* isFireFoxIncognito */])();

      if (info.menuItemId == "removeOL") {
        if (Settings.pb_detectOverlays) {
          if (!isFFP) {
            Object(__WEBPACK_IMPORTED_MODULE_8__utils__["q" /* sendToActiveTab */])('manual-remove-overlay', { source: 'right-click' });
          } else {
            Object(__WEBPACK_IMPORTED_MODULE_8__utils__["q" /* sendToActiveTab */])('overlay-not-active-incognito', { source: 'right-click' });
          }
        } else {
          Object(__WEBPACK_IMPORTED_MODULE_8__utils__["q" /* sendToActiveTab */])('overlay-not-active', { source: 'right-click' });
        }
  ...
  function trackGAEvent(request, isCount) {
    __WEBPACK_IMPORTED_MODULE_1__ga_ch__["a" /* default */].trackEvent(request.category, request.action, request.label, request.dimmesntions);
  }
  ...
  function getCurrentTabData(callback) {
    chrome.tabs.query({ currentWindow: true, active: true }, tabs => {
      ...
      if (Object(__WEBPACK_IMPORTED_MODULE_8__utils__["o" /* isValidUrl */])(tab.url)) {
...
GoogleAnalytics.trackEvent = (category, action, label, dimmesntions = {}) => {
  Object.assign(dimmesntions, {
    dimension4: Object(__WEBPACK_IMPORTED_MODULE_0__utils__["h" /* getManifestInfo */])().version
  });

  ga("send", 'event', category, action, label, dimmesntions);
};
...
class KeenIO {
	...
	//#We'll disable keen.io events for now, we dont really use them and they increased their pricing x6!
