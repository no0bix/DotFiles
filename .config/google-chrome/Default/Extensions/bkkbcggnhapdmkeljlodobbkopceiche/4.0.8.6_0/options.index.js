...
const create_list = options => {
  ...
  const add = (() => {
    var _ref = _asyncToGenerator(function* () {
      ...
      if (validateUrl(pattern_value)) {
        ...
        if (pattern.length > 0) {
          const whitelist = yield __WEBPACK_IMPORTED_MODULE_2__whitelistManager__["a" /* WhitelistManager */].get();
  ...
  function renderList() {
    $(`#${_listtag}`).html($.map(_internal_data_list, item => `<div class='list-group-item'><img class='favicon' src='http://www.google.com/s2/favicons?domain=${item}'><span class="domain">${item} </span><span class='removeButton${_listtag} glyphicon glyphicon-remove'></span></div>`).join(""));
  }
...
$(document).ready(() => {
  ...
  $('#overlay-message').html(Object(__WEBPACK_IMPORTED_MODULE_3__utils__["i" /* getPrivacyPolicy */])(Object(__WEBPACK_IMPORTED_MODULE_3__utils__["g" /* getI18N */])('overlaysEnableMessage')));
  ...
  if (isIncognito) {
    $("#overlay-panel").append("<div class='overlay_block'>Overlay blocking isn't supported in Firefox private mode</div>");
    $("#overlaysToggle").prop('disabled', 'true');
    $("#overlayContextMenuToggle").prop('disabled', 'true');
  }
});
