...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__PBStorageInstance__ = __webpack_require__(3);
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__PBCombinedStorage__ = __webpack_require__(4);
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return WhitelistUpdater; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return WhitelistManager; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__ = __webpack_require__(1);
...
const WhitelistUpdater = {
	init({ whitelist }) {
		if (whitelist) {
			whitelist.defaultPopups = __WEBPACK_IMPORTED_MODULE_1__const__["a" /* default */].defaultWhiteList.popups;
		}
	...
	getDefaultPopups() {
		return new Promise((() => {
			var _ref = _asyncToGenerator(function* (resolve) {
				const data = yield __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_remoteWhitelist.get();
	...
	get() {
		return new Promise((() => {
			var _ref2 = _asyncToGenerator(function* (resolve) {
				const { defaultSites: def, userDefaultExclude: exclude, userWhitelist: user, defaultPopups: popups } = yield __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_whitelistManager.get();
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__content_content__ = __webpack_require__(20);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ocl__ = __webpack_require__(26);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ocl___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1__ocl__);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__overlayKiller__ = __webpack_require__(21);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__inject__ = __webpack_require__(22);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__ntfShowManager__ = __webpack_require__(23);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__notification__ = __webpack_require__(24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__blockCountManager__ = __webpack_require__(25);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__whitelistManager__ = __webpack_require__(8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__const__ = __webpack_require__(0);
...
(() => {
  var _ref = _asyncToGenerator(function* ($) {
    const tabUrl = yield Object(__WEBPACK_IMPORTED_MODULE_9__utils__["j" /* getTabUrl */])();

    const pbMessage = Object(__WEBPACK_IMPORTED_MODULE_9__utils__["p" /* randId */])();

    const ntfShowManager = new __WEBPACK_IMPORTED_MODULE_3__ntfShowManager__["a" /* default */]();

    const blockCountManager = new __WEBPACK_IMPORTED_MODULE_5__blockCountManager__["a" /* default */]();

    const notification = new __WEBPACK_IMPORTED_MODULE_4__notification__["a" /* default */]({
      onHideClick({ source }) {
        ntfShowManager.updateDontShow({ domain: tabUrl.domain });
      ...
      onDisplay({ source }) {
        switch (source) {
          ...
          case 'overlay-not-found':
            sendGoogleAnalyticsEvent({ category: 'Not_found_notification', action: 'Shown', label: tabUrl.domain });
            sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:not-found', post: { 'domain': tabUrl.domain, 'url': tabUrl.href } } });
            break;

          case 'rating':
            sendGoogleAnalyticsEvent({ category: 'rankUs', action: 'Shown', label: tabUrl.domain, isCount: true });
            break;

          case 'quick-tip':
            sendGoogleAnalyticsEvent({ category: 'tip_notification', action: 'Shown', label: tabUrl.domain });
            break;

          case 'overlay-not-supported':
            sendGoogleAnalyticsEvent({ category: 'Notification_overlay_not_active', action: 'Shown', label: tabUrl.domain });
            break;
        }
      },
      ...
      onCloseClick({ source }) {
        switch (source) {
          case 'popup-blocked':
            sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Click', label: 'Close', domain: tabUrl.domain });
            break;
          ...
          case 'overlay-not-supported':
            sendGoogleAnalyticsEvent({ category: 'Notification_overlay_not_active', action: 'Click', label: 'Close' });
            break;
        }
      },

      onButtonClick({ source, id }) {
        switch (source) {
          case 'popup-blocked':
            switch (id) {
              case 'allowOnce':
                sendGoogleAnalyticsEvent({ category: 'Notification_bar', action: 'Click', label: 'Allow once' });
                //sendMessageToBackground({name:'KeenIO', data:{event: 'block-pop-ntf:allow-once', post: {'domain': tabUrl.domain, 'url': tabUrl.href}}});
                break;
          ...
          case 'removed-overlay':
            switch (id) {
              case 'undoLabel':
                ...
                sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:undo', post: { 'domain': tabUrl.domain, 'url': tabUrl.href } } });
                break;
          ...
          case 'overlay-not-found':
            switch (id) {
              case 'TS_Report':
                ...
                sendMessageToBackground({ name: 'KeenIO', data: { event: 'report-overlay-not-found', post: { 'domain': tabUrl.domain, 'url': tabUrl.href } } });
                break;
            }
            break;
          ...
          case 'overlay-not-supported':
            switch (id) {
              case 'NTF_OVERLAY_AGREE':
                sendGoogleAnalyticsEvent({ category: 'Notification_overlay_not_active', action: 'Click', label: 'Agree' });
                break;
            }
    ...
    const overlayKiller = new __WEBPACK_IMPORTED_MODULE_1__overlayKiller__["a" /* default */]({
      onUserRemove({ success, recepies = [], props = {} }) {
        if (success) {
          showOverlayRemovedNotifiction({ recepies, props });
      ...
      onRecipeFound({ found }) {
        ntfShowManager.isShow({ domain: tabUrl.domain, type: 'overlay' }).then(() => {
          showRecipeOverlayNortification();
        });

        updateBlockCounts({ block: 'overlay' }, found.length);
      },
      onBlockListFound({ found }) {
        ntfShowManager.isShow({ domain: tabUrl.domain, type: 'overlay' }).then(() => {
      ...
      onRecipeNotFound(notFound) {
        notFound.forEach(item => {
          //sendToBackground('KeenIO', {event: 'overlay-recipe-not-found', post: {'domain': tabUrl.domain, 'url': tabUrl.href, 'recipe': btoa(item.recipe)}});
        });
      }
    });
      ...
      chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        switch (request.name) {
          ...
          case 'receivedRecipes':
            let recepies = (request.data || {}).recipes || [];
          ...
          case 'manual-remove-overlay':
            ...
            break;
          case 'overlay-not-active':
            showNotSupportedPopup({ source: request.data.source });
            break;
          case 'overlay-not-active-incognito':
            showNotSupportedIncognitoPopup({ source: request.data.source });
            break;
          case "getReportData":
          ...
          case "toggleFunction":
            if (request.toggle) {
              ...
              Object(__WEBPACK_IMPORTED_MODULE_9__utils__["d" /* executeCode */])(`window.pbExternalCommand && window.pbExternalCommand(1, "${pbMessage}");`);
            } else {
              Object(__WEBPACK_IMPORTED_MODULE_9__utils__["d" /* executeCode */])(`window.pbExternalCommand && window.pbExternalCommand(0, "${pbMessage}");`);
            }
            break;
          case 'showRateUs':
            showRateUsPopup();
    ...
    function monitorBlocks() {
      ...
      window.addEventListener("message", (event = {}) => {
        ...
        if (data && data.type == "blockedWindow") {
          ...
          ntfShowManager.isShow({ domain: tabUrl.domain }).then(() => {
            showPopupBlockedNotification(args, { domain: tabUrl.domain });
          });
          ...
          sendGoogleAnalyticsEvent({ category: 'General', action: 'Blocked_popups', label: tabUrl.domain });
        }
      }, false);
    }
    ...
    function setPendingNotificationParams(args) {
      let pop = (args[0] || '') + "|" + (args[1] || '') + "|" + (args[2] || '');
      let relatedLink = args[3] || '';
      let currentHost = location.host;
      let popHost = new URL(Object(__WEBPACK_IMPORTED_MODULE_9__utils__["e" /* getAbsoluteURL */])(args[0])).host;
      ...
      let isSameHostPopup = popHost == currentHost;
    ...
    function parsePendingNotificationParams() {
      if (window.name && window.name.indexOf('pp_pending_ntf:') == 0) {
        let data = JSON.parse(atob(name.split(':')[1]));
        ...
        if (data.time + 5000 >= Date.now()) {
          ...
          if (data.display) {
            ntfShowManager.isShow({ domain: tabUrl.domain }).then(() => {
              showPopupBlockedNotification(data.pop.split('|'), { preventDisplayEvent: true, quickOpen: true, domain: tabUrl.domain });
            });
    ...
    function showPopupBlockedNotification(args, props = {}) {
      ...
      notification.show({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('notificationMessage'),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_ON_SITE') + ' ' + domain,
        ...
        buttons: {
          ...
          allowAlways() {
            sendMessageToBackground({ name: 'allowAlways', domain: tabUrl.domain, type: 'popup' });
            sendMessageToBackground({ name: 'KeenIO', data: { event: 'allow-always:pops', post: { 'domain': tabUrl.domain, 'url': tabUrl.href, 'pop': args[0] } } });
    ...
    function showRecipeOverlayNortification() {
      notification.show({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])("overlayWasBlocked"),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_ON_SITE') + ' ' + tabUrl.domain,
    ...
    function showOverlayRemovedNotifiction({ recepies, props: { source } }) {
      notification.show({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('overlayWasRemoved'),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])("NTF_ON_SITE") + ' ' + tabUrl.domain,
        icon: chrome.runtime.getURL('/images/logo-20x20.svg'),
        showResize: true,
        ...
        buttons: {
          ...
          removeAlwaysLabel() {
            overlayKiller.addToLocalBlacklist({ domain: tabUrl.domain, url: tabUrl.href });

            sendMessageToBackground({ name: 'KeenIO', data: { event: 'overlay-ntf:remove-always', post: { 'domain': tabUrl.domain, 'url': tabUrl.href, recepies: btoa(recepies) } } });
          }
        },
        onDisplay() {
          sendMessageToBackground({ name: 'update-rating', action: 'display' });
        ...
        onClose: function () {
          if (source == 'menu') {
            ntfShowManager.isShow({ domain: tabUrl.domain, type: 'tip' }).then(() => {
    ...
    function showNoOverlayFoundNotification() {
      notification.show({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('TS_NoOverlay'),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_ON_SITE') + ' ' + tabUrl.domain,
        icon: chrome.runtime.getURL('/images/logo-20x20.svg'),
        ...
        buttons: {
          TS_Report() {
            reportNoOverlayFound(tabUrl.href, tabUrl.domain);
          }
        }
      });
    }
    ...
    function showOverlayTipFromMenu() {
      notification.addQueue({
        stopQueue: true,
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_QUICK_TIP'),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_TIP_OVERLAY_MENU'),
    ...
    function showRateUsPopup() {
      notification.addQueue({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])("rateus"),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])("TS_Rate_ShowLove"),
        icon: false,
        persistent: true,
        showStars: true,
        size: 'maximized',
        ...
        onRating: function (score) {
          if (score == 5) {
            openUrl(__WEBPACK_IMPORTED_MODULE_8__const__["a" /* default */]['extension_review_page_' + Object(__WEBPACK_IMPORTED_MODULE_9__utils__["f" /* getBrowser */])()]);
          } else {
            openUrl(__WEBPACK_IMPORTED_MODULE_8__const__["a" /* default */].contact_page);
          }
    ...
    function showNotSupportedPopup(options) {
      notification.addQueue({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_OVERLAY_INACTIVE_TITLE'),
        subTitle: '<div class="overlay-not-supported">' + Object(__WEBPACK_IMPORTED_MODULE_9__utils__["i" /* getPrivacyPolicy */])(Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_OVERLAY_INACTIVE_MESSAGE_' + Object(__WEBPACK_IMPORTED_MODULE_9__utils__["f" /* getBrowser */])())) + '</div>',
        icon: chrome.runtime.getURL('/images/ntf/warning.svg'),
        persistent: true,
        size: 'maximized',
        cssClass: 'pb-toast-overlay',
        source: 'overlay-not-supported',
        buttons: {
          NTF_OVERLAY_AGREE() {
            sendMessageToBackground({ name: "NotificationOptIn" });

            overlayKiller.remove({ source: options.source });
          }
        }
      });
    }

    //overlay not supported in incognito firefox
    function showNotSupportedIncognitoPopup(options) {
      notification.addQueue({
        title: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_OVERLAY_INACTIVE_TITLE'),
        subTitle: Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])('NTF_OVERLAY_UNAVAILABLE_MESSAGE'),
        icon: chrome.runtime.getURL('/images/ntf/warning.svg'),
        persistent: true,
        size: 'maximized',
        source: 'overlay-not-supported',
    ...
    function updateBlockCounts(blockInfo, counts = 1) {
      blockCountManager.updateCounts({ blockInfo, counts }).then(function (data) {
        sendMessageToBackground({ name: "updateBlockCounts", numOfBlocks: data.numOfBlocks });

        __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_hideBadge.get().then(show => {
          if (!show) {
    ...
    function sendGoogleAnalyticsEvent(data) {
      const { domain = tabUrl.domain } = data;
    ...
    function initBlockFunctions() {
      __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].many(__WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_hideBadge, __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_overlaylist, __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_pause, __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_popupBlackList).get().then((() => {
        var _ref2 = _asyncToGenerator(function* (settings) {
          ...
          const whitelist = yield __WEBPACK_IMPORTED_MODULE_6__whitelistManager__["a" /* WhitelistManager */].get();

          if (!Object(__WEBPACK_IMPORTED_MODULE_9__utils__["l" /* isDomainInList */])(tabUrl.domain, whitelist.sites.all)) {
            let code = `(function () {
          var pb_message = "${pbMessage}";
          var pb_blacklist = ${JSON.stringify(settings.pb_popupBlackList)};
          var pb_whitelist = ${JSON.stringify(whitelist.popups)};
          ${__WEBPACK_IMPORTED_MODULE_2__inject__["a" /* inject */].toString()};
          inject();
        })();`;

            Object(__WEBPACK_IMPORTED_MODULE_9__utils__["d" /* executeCode */])(code);
          ...
          if (!settings.pb_hideBadge && window == top) {
            sendMessageToBackground({ name: "setBadgeText" });
          }
        });
    ...
    function onHyperLinkClicked(e) {
      ...
      let target = el.attr('target') || '_self';
      ...
      if (isExtensionAlive() && href && href !== '#' && target !== '_self') {
        ...
        if (isOverlayish(el)) {
          ...
          __WEBPACK_IMPORTED_MODULE_7__PBStorage_PBStorageSync__["a" /* default */].pb_pause.get().then(isPause => {
            if (!isPause) {
              parent.postMessage({
                type: "blockedWindow",
    ...
    function isExtensionAlive() {
      try {
        return Object(__WEBPACK_IMPORTED_MODULE_9__utils__["g" /* getI18N */])("options");
      } catch (e) {
        return false;
      }
    }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__ = __webpack_require__(1);
...
class OverlayDectector {
  ...
  ALGO1() {
    let children = document.querySelectorAll('body > :not(script)');
    let center = { x: this.winWidth / 2, y: this.winHeight / 2 };
    let candidates = [],
        winner;
    let overlay;

    children.forEach(item => {
      let res = this.isOverlayish(item);
  ...
  ALGO2({ x, y, mw, mh }) {
    ...
    if (element && this.notRootElement(element)) {
      do {
        let isOverlayish = this.isOverlayish({ el: element, minWidth: mw, minHeight: mh });
  ...
  getRecipe(el) {
    let recipe = [];
    const isSingleInstanceInDocument = recipe => {
      return __WEBPACK_IMPORTED_MODULE_0_jquery___default()(recipe.join(' > ')).length == 1;
    };
  ...
  hideByRecipe(recipe) {
    let el = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(recipe);
    let css = el.prop('style').cssText;

    el.prop('style').cssText = `${css};display:none !important;`;

    return { el, recipe, css };
  }

  isOverlayish({ el, minWidth, minHeight }) {
...
class OverlayKiller extends OverlayDectector {
  constructor(props = {}) {
    super();

    this.activeBlockList = [];
    this.activeRecepiesList = [];

    this.onAutoDetect = props.onAutoDetect || (() => {});
    this.onUserRemove = props.onUserRemove || (() => {});
    this.onBlockListFound = props.onBlockListFound || (() => {});
    this.onRecipeFound = props.onRecipeFound || (() => {});
    this.onRecipeNotFound = props.onRecipeNotFound || (() => {});

    this.blackListPromise = this.createBlockListFromStorage();

    __WEBPACK_IMPORTED_MODULE_0_jquery___default()(_ => {
      if (window == top) {
        __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__["a" /* default */].pb_pause.get().then(isPause => {
          if (!isPause) {
            this.autoDetect();
          }
        });
  ...
  restore() {
    this.last.forEach(item => {
      let { el, recipe, css } = item;

      el.prop('style').cssText = css + ';display: block';

      [...__WEBPACK_IMPORTED_MODULE_0_jquery___default()('style.pp-remove-overlay')].forEach(style => {
        style = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(style);

        if (style.data('recipe') == btoa(recipe)) {
          __WEBPACK_IMPORTED_MODULE_0_jquery___default()(style).remove();
        }
      });
    });
  ...
  remove(props = {}) {
    ...
    if (overlays.length) {
      ...
      this.onUserRemove({ success: true, recepies: overlays.map(o => o.recipe), props });
    } else {
      this.onUserRemove({ success: false, props });
    }
  }

  // ===== add found overlay to local blacklist, <remove always> button from notification
  addToLocalBlacklist({ domain, url }) {
  ...
  removeFromLocalBlacklist() {
    ...
    __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__["a" /* default */].pb_overlayBlockedList.update((list = {}) => {
      let blocks = list[domain] || [];
      let lastRecepies = this.last.map(item => item.recipe);
  ...
  autoDetect(attempt = 0) {
    if (attempt < 100) {
      let foundBlockList = this.forceBlock(this.activeBlockList);
      let foundRecipeList = this.forceBlock(this.activeRecepiesList);

      if (foundBlockList.length) {
        setTimeout(() => this.onBlockListFound({ found: foundBlockList }), 750); //setTimout for UX purpuses
      }

      if (foundRecipeList.length) {
        setTimeout(() => this.onRecipeFound({ found: foundRecipeList }), 750); //setTimout for UX purpuses
      }
      ...
      //after 100 attempts we insert css block to maybe block overlays that appear after that
      this.createBlockCSS(this.activeBlockList.concat(this.activeRecepiesList));
    ...
    return new Promise(resolve => {
      __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__["a" /* default */].pb_overlayBlockedList.get().then((list = {}) => {
        if (list[document.domain]) {
          this.activeBlockList = list[document.domain];
  ...
  createBlockListFromRecepies(recepies) {
    ...
    this.blackListPromise.then(() => {
      ...
      this.activeRecepiesList = recepies;
  ...
  //#SUPER IMPORTANT!!!! - MAKE SURE THIS NOT IS NOT RUNNING ON EVERY PAGE
  //IT MAKES THE SCROLL TO JUMP TO START
  //RUN ONLY ON SITES THE USER REMOVED AN OVERLAY OR HAVE A RECIPE
  //AND ONLY ON ON PAGE START!!!
  createGlobalCSS() {
    if (!this.globalCSS) {
      const htmlStyle = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('html').attr('style');
      const bodyStyle = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('body').attr('style');

      __WEBPACK_IMPORTED_MODULE_0_jquery___default()('<style type="text/css">html, body {overflow:auto !important; }</style>').appendTo('head');
      __WEBPACK_IMPORTED_MODULE_0_jquery___default()('html').attr('style', (htmlStyle ? htmlStyle + ';' : '') + 'overflow:auto !important;');
      __WEBPACK_IMPORTED_MODULE_0_jquery___default()('body').attr('style', (bodyStyle ? bodyStyle + ';' : '') + 'overflow:auto !important;');
  ...
  //#removed - bad user experience, needs to see the actual overlay being removed
  //#return - run this after 100 attemts, better to block then not to block 
  createBlockCSS(list) {
    //this.createGlobalCSS();  #see "SUPER IMPORTANT" comment for reason of removal

    list.forEach(item => {
      let css = item.recipe + ' { display:none !important; }';

      __WEBPACK_IMPORTED_MODULE_0_jquery___default()(`<style class="pp-remove-overlay" data-recipe="${btoa(item.recipe)}" type="text/css">${css}</style>`).appendTo('head');
    });
  }
  ...
  forceBlock(list) {
    ...
    list.filter(item => !item.found).forEach(item => {
      let overlay = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(item.recipe);
    ...
    if (hide.length) {
      setTimeout(() => this.hide(hide), 750); //setTimout for UX purpuses
    }
...
/* harmony default export */ __webpack_exports__["a"] = (OverlayKiller);
...
/* unused harmony export restoreWindowOpen */
/**
...
class NtfShowManager {
  constructor() {
    this.isActive = { pop: true, overlay: true, tip: true };
  }

  isShow(props = {}) {
    const { domain, type = 'pop' } = props;

    return new Promise(resolve => {
      __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].many(__WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_hideNotifications, __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].doNotShowNotifyList, __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_lastNotificationDisplay).get().then(settings => {
        const isGlobalHidden = settings.pb_hideNotifications;
        const hiddenSitesList = settings.doNotShowNotifyList;
        const lastDisplay = settings.pb_lastNotificationDisplay || {};

        if (this.isActive[type] && this.checkLastDisplay({ domain, type, lastDisplay }) && !isGlobalHidden && !this.checkIsHidden({ hiddenSitesList, domain })) {
          this.isActive[type] = false;

          this.updateLastDisplay({ domain, type });
          resolve();
        }
      });
    });
  ...
  checkIsHidden({ hiddenSitesList, domain }) {
    const item = hiddenSitesList.find(item => item.domain == domain);

    if (item && item.time) {
      const time = new Date(item.time);
      const time30days = new Date(time.setDate(time.getDate() + 30));
      const now = new Date();
  ...
  checkLastDisplay({ domain, type, lastDisplay }) {
    let now = new Date().getTime();
    let last = lastDisplay[type];
  ...
  updateLastDisplay({ domain, type }) {
    __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_lastNotificationDisplay.update((display = {}) => {
      switch (type) {
        case 'overlay':
  ...
  updateDontShow({ domain }) {
    __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].doNotShowNotifyList.update(doNotShowList => {
      doNotShowList = doNotShowList.filter(item => item.domain != domain);
      doNotShowList.push({
        domain: domain,
...
/* harmony export (immutable) */ __webpack_exports__["a"] = NtfShowManager;
...
class Notification {
  constructor(props = {}) {
    ...
    __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__["a" /* default */].pb_notification_size.get().then(data => {
      if (data) {
        this.notificationSize = data.size;
      }
    });

    window.addEventListener('message', event => {
      if (chrome.runtime.getURL('').indexOf(event.origin) == 0 && event.data.toastId == this.toastId) {
        let data = event.data;

        switch (data.action) {
          case 'pb-message-display':
            this.options.onDisplay && this.options.onDisplay();

            this.props.onDisplay.call(this, { source: data.source });
            break;

          case 'message-resize':
            this.toast.toggleClass('pb-toast-minimized pb-toast-maximized');
            this.notificationSize = data.resize;

            __WEBPACK_IMPORTED_MODULE_1__PBStorage_PBStorageSync__["a" /* default */].pb_notification_size.set({ size: data.resize });

            this.props.onResizeClick.call(this, { size: data.resize });
            break;

          case 'message-hide':
            this.props.onHideClick.call(this, { source: data.source });

            this.queueFade(100);
            break;

          case 'pb-message-close':
            this.userGeneratedClose = true;

            //remove immediately
            if (data.quickClose) {
              this.remove();
            } else {
              this.queueFade(100, this.options.onFadeOut);
            }

            if (data.triggerEvent) {
              this.options.onClose && this.options.onClose();
              this.options.onButtonClick && this.options.onButtonClick({ id: 'close' });

              this.props.onButtonClick.call(this, { source: data.source, id: 'close' });
              this.props.onCloseClick.call(this, { source: data.source });
            }
            break;

          case 'pb-message-btn-click':
            if (this.options.buttons[data.id]) {
              this.options.buttons[data.id]();
            }

            this.options.onButtonClick && this.options.onButtonClick({ id: data.id });
            this.props.onButtonClick.call(this, { source: data.source, id: data.id });
            break;

          case 'pb-message-tooltip-over':
            if (Object(__WEBPACK_IMPORTED_MODULE_2__utils__["n" /* isI18N */])(data.id)) {
          ...
          case 'pb-message-rating':
            ...
            if (this.options.onRating) {
              this.options.onRating && this.options.onRating(data.score);

              this.props.onButtonClick.call(this, { source: data.source, id: 'rating' });
            }
            break;

          case 'pb-message-share':
            if (this.options.onShare) {
              this.options.onShare && this.options.onShare(data.media);
              this.props.onButtonClick.call(this, { source: data.source, id: 'rating' });

              if (data.media == 'link') {
                this.showLinkCopied(data.position);
              }
            }
            break;
        }
    ...
    document.addEventListener('visibilitychange', event => {
      if (document.visibilityState == 'visible') {
        ...
        if (this.queueFadeTime && now > this.queueFadeTime + 5000) {
          this.remove();
        }
      }
    }, false);
  ...
  render() {
    ...
    this.toast = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('<iframe id="pb-toast-main" class="' + this.getCSSClass() + '">').on('load', () => {
      if (this.options.quickOpen) {
        this.toast.show().addClass('pb-toast-main-show');
      } else {
        this.toast.show().addClass('pb-toast-main-move');
      }

      if (!this.options.persistent) {
        this.queueFadeTime = new Date().getTime();
        this.queueFade(5000, this.options.onFadeOut);
      }
    }).on('mouseenter', () => {
      if (!this.options.persistent) {
        this.cancelFade();
      ...
      if (!this.userGeneratedClose && !this.options.persistent) {
        this.queueFade(5000, this.options.onFadeOut);
      }
    }).on('transitionend', () => {
  ...
  init(options) {
    ...
    this.toastId = Object(__WEBPACK_IMPORTED_MODULE_2__utils__["a" /* GUID */])();
    this.options = options;
  ...
  queueFade(time, onFadeOut) {
    this.fid = setTimeout(() => {
      this.toast.attr('class', this.getCSSClass());
  ...
  showTooltip({ id, position }) {
    let toastPosition = this.toast.get(0).getBoundingClientRect();
    let tooltipPosition = {
      left: toastPosition.left + position.xCenter,
      top: toastPosition.top + position.yCenter + 15
    };
    let tooltip = this.renderTooltip(id);
  ...
  hideTooltip() {
    __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#pb_jq_tipsWrapper').remove();
  }

  renderTooltip(id) {
    this.hideTooltip();

    return __WEBPACK_IMPORTED_MODULE_0_jquery___default()('<div id="pb_jq_tipsWrapper">' + Object(__WEBPACK_IMPORTED_MODULE_2__utils__["g" /* getI18N */])(id) + '</div>').appendTo('body');
  }
  ...
  showLinkCopied(iconPosition) {
    ...
    __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#pb-link-copied-message').remove();
    let msg = __WEBPACK_IMPORTED_MODULE_0_jquery___default()('<div id="pb-link-copied-message">' + Object(__WEBPACK_IMPORTED_MODULE_2__utils__["g" /* getI18N */])('NTF_LinkCopied') + '</div>').css({
      top: position.top + 31,
    ...
    setTimeout(() => msg.animate({ opacity: 0 }, 250, () => msg.remove()), 2000);
  }
...
class BlockCountManager {
    updateCounts(props) {
        const { blockInfo, counts = 1 } = props;

        return new Promise(resolve => {
            __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].many(__WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_numOfBlocks, __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_counterBlockedSites, __WEBPACK_IMPORTED_MODULE_0__PBStorage_PBStorageSync__["a" /* default */].pb_totalBlocksByType).update(settings => {
                settings.pb_numOfBlocks++;
                settings.pb_totalBlocksByType = this.updateBlockedByType({ blocks: settings.pb_totalBlocksByType, blockInfo, counts });

                let curDomain = location.host;
                let pb_BC = settings.pb_counterBlockedSites || {};
                let data = pb_BC[curDomain] || { todayCount: 0, count: 0, currentTimeRange: this.currTR() };

                data.lastUpdated = Date.now();
                data.count = data.count + counts;

                if (this.isBetweenTimeRange(data.currentTimeRange)) {
                    data.todayCount = data.todayCount + counts;
                } else {
                    data.currentTimeRange = this.currTR();
    ...
    currTR() {
        let now = new Date();
        let todayStart = new Date(now.getFullYear(), now.getMonth(), now.getHours() >= 0 && now.getHours() < 5 ? now.getDate() - 1 : now.getDate(), 5, 0, 1); //day starts from 5PM
        let todayEnd = new Date(todayStart.getFullYear(), todayStart.getMonth(), todayStart.getDate() + 1, 5, 0, 0); //day end at 5PM next day

        return [todayStart.getTime(), todayEnd.getTime()];
    }

    isBetweenTimeRange(dateRange) {
        let now = new Date();
        let start = new Date(dateRange[0]);
        let end = new Date(dateRange[1]);
    ...
    updateBlockedByType({ blocks, blockInfo, counts }) {
...
/* harmony export (immutable) */ __webpack_exports__["a"] = BlockCountManager;
...
const cs = chrome.runtime.sendMessage;
const dba = window.addEventListener.bind(window),
      locateHref = function (e) {
  ...
  const href = locateHref(e.target);
  if (href) {
    cs({ ahref: href });
  }
},
